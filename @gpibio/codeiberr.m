function [errcode, errdesc] =codeiberr(gpib,err)
    % codeiberr - returns the error mnemonic for a given error code
    % [errcode, errdesc] =gpib.codeiberr(iberr)
    %
    % iberr - iberr returned by ThreadIberr
    %
    % errcode - error code mnemonic for the value of iberr
    % errdesc - error code description
    
    codes = {'EDVR', 'A system call has failed. ibcnt/ibcntl will be set to the value of errno.'; ...
             ...
             'ESRQ', 'The serial poll request service line is stuck on. This can occur if a physical device on the bus requests service, but its GPIB address has not been opened (via ibdev() for example) by any process. Thus the automatic serial polling routines are unaware of the devices existence and will never serial poll it.'; ...
